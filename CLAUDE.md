...
- Students complete four lab reports (25% each; rubric 50/30/20). This repo has no `assignments/`; `labs/` takes its place. `labs/index.qmd` lists the 13 available experiments and is a third `index.qmd`, alongside the root and `lectures/` ones; there are no per-assignment pages. Add `labs/<code>.qmd` pages as they are written.
...
- Canonical schedule is in the root `index.qmd`; the calendar PDF is the source. Keep it in sync with the syllabus.
